...
const aboutRoutes = [
  {
    label: "Our Story",
    href: "/about-acadally",
    desc: "Our mission to transform learning",
  ...
  },
  {
    label: "FAQ",
    href: "/faq",
    desc: "Quick answers to your queries",
...
const featureRoutes = [
  {
    label: "Students",
    href: "/students",
    desc: "Personalized path to success",
    icon: <PiStudent className="w-5 h-5" />,
  },
  {
    label: "Schools",
    href: "/schools",
    desc: "AI solutions for institutions",
    icon: <RiSchoolLine className="w-5 h-5" />,
  },
  {
    label: "Teachers",
    href: "/teachers",
    desc: "Tools for modern educators",
...
const navLinks = [
  {
    ...
    href: "#",
    hasDropdown: true,
    dropdownItems: aboutRoutes,
  },
  {
    label: "Features",
    href: "#",
    hasDropdown: true,
    dropdownItems: featureRoutes,
  },
...
export default function Navbar() {
  ...
  // Map pathname to page label
  function getPageLabel(pathname: string) {
    if (pathname === "/" || pathname === "") return "Home";
    // Check navLinks
    for (const link of navLinks) {
      if (link.href !== "#" && link.href === pathname) return link.label;
    ...
    // Fallback: use last part of path, capitalized
  ...
  return (
    <>
      <nav
        className={cn(
          "fixed top-0 left-0 right-0 z-100 flex items-center py-4 px-4 lg:px-20 transition-all duration-300",
          isScrolled
          ...
          <div className="flex items-center gap-3 lg:gap-8">
            ...
            <div className="md:hidden flex items-center gap-3">
              ...
              <div
                ...
                className="cursor-pointer w-6 h-5 flex flex-col justify-between"
              >
                <span
                  className={`h-0.5 bg-main-page-secondary transition-all ${
            ...
            <div className="hidden md:flex items-center gap-4 lg:gap-8 md:text-base md:font-medium text-gray-800">
              {navLinks.map((link, index) => {
                ...
                if (link.hasDropdown && link.dropdownItems) {
                  const activeItem = link.dropdownItems.find(
                    (item) => item.href === pathname,
                  );
                  if (activeItem) activeDropdownLabel = activeItem.label;
                }

                const isActive =
                  pathname === link.href || !!activeDropdownLabel;
                ...
                return (
                  ...
                  >
                    <Link
                      href={link.href}
                      className={`flex items-center hover:text-main-page-secondary transition-colors ${
                        isActive ? "text-main-page-secondary font-bold" : ""
                      }`}
                    >
                    ...
                    {link.hasDropdown && openDropdown === link.label && (
                      <div
                        className="absolute top-full left-0 w-72 pt-4 z-50"
                        onMouseEnter={() => handleMouseEnter(link.label)}
                        onMouseLeave={handleMouseLeave}
                      >
                        <div className="bg-white rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.15)] border border-gray-100 p-2">
                          {link.dropdownItems?.map((route, i) => (
                            <Link
                              key={i}
                              href={route.href}
                              className="flex items-start gap-4 p-3 rounded-xl hover:bg-main-page-secondary/5"
                            >
              ...
              {link.hasDropdown && openDropdown === link.label && (
                <div className="mt-2 grid grid-cols-1 gap-1">
                  {link.dropdownItems?.map((route, i) => (
                    ...
                    >
                      <div
                        className={`w-8 h-8 rounded-lg flex items-center justify-center ${pathname === route.href ? "bg-main-page-secondary text-white" : "bg-gray-100 text-gray-400"}`}
                      >
                        {route.icon}
                      </div>
                      <div className="flex flex-col">
                        <span
                          className={`text-sm ${pathname === route.href ? "font-bold" : "font-semibold"}`}
                        >
                          {route.label}
                        </span>
