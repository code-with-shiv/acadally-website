                    ...
                    <div className="lg:w-1/2 flex flex-col bg-white">
                        <div className="p-5 md:p-8 flex-1 text-xs md:text-sm">
                            ...
                            <p className="text-gray-600 text-[12px] leading-[140%] font-normal md:text-sm md:font-medium md:leading-relaxed mb-4">
                                <span className="text-[#1C4CC3] font-bold">AcadAlly</span> participated in the <span className="font-semibold">{latestEvent.title}</span> in {latestEvent.location} on {latestEvent.date}. {latestEvent.description}
                            </p>
                            ...
                            <div className="space-y-2">
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-[#535353] text-[14px] leading-[24px] font-normal md:text-gray-600 md:font-normal truncate">{latestEvent.location}</span>
                                </div>
                                <div className="flex items-center gap-2">
                                    <div className="w-6 h-6 md:w-8 md:h-8 rounded-lg bg-[#EBF2FF] flex items-center justify-center shrink-0">
                                        <Image src="/event-date-calendar-icon.svg" alt="Calendar" width={14} height={14} />
                                    </div>
                                    <span className="text-[#535353] text-[14px] leading-[24px] font-normal md:text-gray-600 md:font-normal">{latestEvent.date}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        {/* Footer Section */}
                        <Link 
                            href={`/event?id=${latestEvent.id}`}
                            className="bg-[#1C4CC314] p-3 md:px-8 md:py-3 flex justify-end items-center border-t border-[#1C4CC3]/10 transition-colors duration-300 w-full mt-auto group-hover:bg-[#1C4CC3]"
                        >
