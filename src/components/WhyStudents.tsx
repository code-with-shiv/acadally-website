...
export default function WhyStudents() {
    const icons1 = ["/students-d-3.png", "/students-d-20.svg", "/students-d-5.svg", "/students-d-6.png"];
    const icons2 = ["/students-d-7.svg", "/students-d-21.svg", "/students-d-9.svg", "/students-d-10.svg"];
    ...
    const card1 = (
        ...
        >
            <div className="flex justify-between lg:justify-between items-center w-full mt-1 gap-2">
                {icons1.map((icon, i) => (
                    <Image key={i} src={icon} alt="icon" width={32} height={32} className="w-6 md:w-8 md:w-14 h-auto transition-transform duration-300 ease-out lg:group-hover:scale-110 lg:group-hover:-translate-y-1" />
                ))}
            </div>
        </WhyCard>
    );
